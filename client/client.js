...
room.controller('Ctrl',['$scope','Room',function ($scope,Room) {
	...
	$scope.$on('localVideo.update', function (e, user) {
		if (Room.localStream)
			document.getElementById("video-" + Room.videosIds[user]).srcObject = Room.localStream;
		else
	...
	$scope.$on('userVideo.update', function (e,user) {
		$scope.$apply($scope.users[user].video = Room.users[user].streams.map(function(stream, i) { 
			document.getElementById("video-" + Room.videosIds[user]).srcObject = stream;
			return stream;
		}));
	});
	...
	$scope.$on('lives.update', function (e) {
		$scope.$apply($scope.lives = Room.lives);
	});

	$scope.$on('cardsByUser.update', function (e) {
		$scope.$apply($scope.cardsByUser = Room.cardsByUser);
	});

	$scope.$on('cardPlayed.update', function (e) {
		$scope.$apply($scope.cardPlayed = Room.cardPlayed);
	});
	...
	$scope.cardsByUser = Room.cardsByUser;
	$scope.cardPlayer = Room.cardPlayer;
	$scope.msg = "";

	$scope.getCards = function(user) {
		var vIds = this.room.videosIds;
		var username;
		for (var i = 0; i < Object.keys(vIds).length; i++) {
			var currentuser = Object.keys(vIds)[i];
			if (currentuser != "undefined" && vIds[currentuser] === user) {
				username = currentuser;
				break;
			}
		}
		return (this.cardsByUser[username] || []).filter((x) => !!x);
	}

	$scope.getOfuscatedCards = function(user) {
		var vIds = this.room.videosIds;
		var username;
		for (var i = 0; i < Object.keys(vIds).length; i++) {
			var currentuser = Object.keys(vIds)[i];
	...
	var room = {
		...
		init : function() {
			...
			$(document).keyup(function(e) {
				if (e.keyCode == 32) {
					room.socket.emit('shiftcard', room.roomname, room.username);
				}
			});

			$("#begin").click(function() {
				room.socket.emit('begin', room.roomname);
			});

			this.socket.on('cardsByUser', function(cardsByUserJSON) {
				var cardsByUser = JSON.parse(cardsByUserJSON);
				room.cardsByUser = cardsByUser;
				$rootScope.$broadcast('cardsByUser.update')
			});

			this.socket.on('lives.update', function(lives) {
				room.lives = lives;
				$rootScope.$broadcast('lives.update')
			});

			this.socket.on('cardPlayed', function(card) {
				room.cardPlayed = card;
				$rootScope.$broadcast('cardPlayed.update')
			});
			
			this.socket.on('userlist', function(userlistJSON) {
		...
		onMediaSuccess : function(stream) {
			...
			if (oldStream)
				oldStream.stop();
			$rootScope.$broadcast('localVideo.update', room.username);
		},
